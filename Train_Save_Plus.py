...
mnist = input_data.read_data_sets(mnsit_local_path, one_hot=True) #导入本地mnsit数据集，独热模式，这是实际的训练和测试数据

#创建模型 实验三层前馈神经网络结构，网络结构的选择要根据数据集的维度、需求精度以及训练主机的算力的综合情况
#选取合适的神经网络机构层级
...
with tf.name_scope('layer'):#定义权重与偏移量
    with tf.name_scope('W'):#权重
        ...
        #W1 = tf.Variable(tf.truncated_normal([784, 300],stddev=0.1), name='Weights1')  # 权重矩阵张量1
        W1 = tf.Variable(tf.truncated_normal([784, 300],mean=0.0,stddev=0.1),name='Weights1')  # 权重矩阵张量1
        W2 = tf.Variable(tf.truncated_normal([300, 200],mean=0.0,stddev=0.1), name='Weights2')  # 权重矩阵张量2
        W3 = tf.Variable(tf.zeros([200, 10]), name='Weights2')  # 权重矩阵张量2
    with tf.name_scope('b'):#偏移量
        #b = tf.Variable(tf.zeros([10]),name='biases') #偏移量张量
        b1 = tf.Variable(tf.truncated_normal([300],mean=0.0,stddev=0.1), name='biases1')  # 偏移量张量1
        b2 = tf.Variable(tf.truncated_normal([200],mean=0.0,stddev=0.1), name='biases2')  # 偏移量张量2
        b3 = tf.Variable(tf.zeros([10]), name='biases3')   #偏移量张量3
    with tf.name_scope('W_p_b'):#计算公式算子
        #Wx_plus_b = tf.add(tf.matmul(x, W), b, name='Wx_plus_b') #矩阵乘法+偏移量的算子
        h1 = tf.nn.relu(tf.add(tf.matmul(x, W1), b1, name='h1'))  #输入层到隐藏层1算子，使用relu激活函数
        h2 = tf.nn.relu(tf.add(tf.matmul(h1, W2), b2, name='h2'))  #隐藏层1到隐藏层2，使用softmax激活函数
        h3 = tf.add(tf.matmul(h2, W3), b3, name='h3') #隐藏层3到输出，暂不激活
    #keep_prob = tf.placeholder(tf.float32)
    #y = tf.nn.softmax(Wx_plus_b, name='final_result') #输出使用softmax作为激活函数激活算子，注意y 与前面y_的区别
    y = tf.nn.softmax(h3,name='final_result') #输出使用softmax作为激活函数激活算子，注意y 与前面y_的区别
# 定义损失（代价）函数和优化方法
with tf.name_scope('loss'):#损失（代价）函数算子
    loss = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y))) #使用交叉熵作为代价（代价）函数算子
...
with tf.gfile.FastGFile('E:\\BaiduNetdiskDownload\\pycharm\\pycharm练习\\labdata\\md_plus.pb', mode='wb') as f:
    #需要指定模型文件的保存位置
    ###训练的目的就是为了获得模型及其参数数据###
